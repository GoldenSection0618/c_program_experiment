...
## 1. 验收套件
- `menu`：菜单主流程，覆盖添加卡、重复卡号、密码超长、查询存在与不存在，并校验二进制落盘内容
- `menu_add_card_pipe` / `menu_add_password_pipe`：输入包含非法分隔语义字符时拒绝添加，防止非法字符进入持久化数据
...
- `menu_query_missing_file`：二进制数据文件不存在时的查询提示
- `menu_query_empty_file`：空二进制文件按无记录处理，查询返回“没有该卡的信息！”
- `menu_record_format_error`：损坏的二进制记录保护
...
`run_smoke_test.sh` 已支持套件级 `*.setup.sh` / `*.verify.sh` 钩子，用于准备二进制测试文件、校验落盘内容和做重启验证。
